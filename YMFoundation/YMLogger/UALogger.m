...
+ (void)initialize {
  ...
  [self setMinimumSeverity:UALoggerSeverityWarn];
}
...
+ (NSString *)formatForVerbosity:(UALoggerVerbosity)verbosity {
  switch (verbosity) {
    case UALoggerVerbosityNone:
      return nil;
    case UALoggerVerbosityPlain:
      return UA__verbosityFormatPlain;
    case UALoggerVerbosityBasic:
      return UA__verbosityFormatBasic;
    case UALoggerVerbosityFull:
      return UA__verbosityFormatFull;
    default:
      return nil;
  }
}
...
+ (NSString *)labelForSeverity:(UALoggerSeverity)severity {
  switch (severity) {
    case UALoggerSeverityDebug:
      return @"DEBUG";
    case UALoggerSeverityInfo:
      return @"INFO";
    case UALoggerSeverityWarn:
      return @"WARN";
    case UALoggerSeverityError:
      return @"ERROR";
    case UALoggerSeverityFatal:
      return @"FATAL";
    default:
      return @"";
  }
...
+ (BOOL)loggingEnabled {
  // True if...
  return ([UALogger usingSeverityFiltering])
      ||                                // Using severity filtering OR							// severity filtering is done in the logWithVerbosity:severity:formatArgs method
          (![UALogger isProduction] && [UALogger shouldLogInDebug])
      ||            // Debug and logging is enabled in debug OR
          ([UALogger isProduction] && [UALogger shouldLogInProduction])
      ||        // Production and logging is enabled in production OR
